...
func TestVertexScalar() {
	...
	for !Done {
		if first {
			gReader = readfiles.NewGoCFDSolutionReader("assets/solution.gcfd",
				true)
			...
			key = chart.AddContourVertexScalar(vs, 0.30, 1.3, 30)
			// key = chart.AddShadedVertexScalar(vs, 1.80, 1.9)
