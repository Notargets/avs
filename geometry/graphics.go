...
func (cm *ColorMap) GetDrawing(box *BoundingBox) (qm *QuadMesh) {
	...
	*/
	...
	for j := 0; j < qm.Dimensions[1]; j++ {
		...
		for i := 0; i < qm.Dimensions[0]; i++ {
			...
			qm.Geometry[i+qm.Dimensions[0]*j] = *NewPoint(X, Y)
	...
	qm.Box = NewBoundingBox(qm.Geometry)
	return qm
}
